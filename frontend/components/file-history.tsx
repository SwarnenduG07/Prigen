...
interface ReceivedFile {
  ...
  export default function  FileHistory() {
    ...
    const [recevedFiles, setRecevedFile] = useState<ReceivedFile[]>([]);
    ...
    useEffect(() => {
     ...
     const getFileHistory = async () => {
      try {
          ...
          setRecevedFile(res.data.files);
      } catch (e: any) {
          console.log(e.response?.data?.message || 'Failed to fetch files');
  ...
  const filesize = (bytes: number) => {
    ...
    return (
     <main className="flex flex-col justify-center items-center text-center mt-12">
        <div className="mt-14 text-2xl font-thin text-gray-800">
        Check Your File History
      </div>
